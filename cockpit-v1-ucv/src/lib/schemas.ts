...
const VariantMetadataSchema = z.object({
  ...
  // NOTE: This is intentionally an *excerpt* of the full JD.
  // The raw/full JD should live in source-data/ (gitignored) for safety.
  jobDescription: z.string(),
  // Optional: original job posting link
  sourceUrl: z.string().url().optional(),
  // Optional: explicit URL segments for /:company/:role routing.
  // If omitted, the app/CLI can derive from metadata.slug using a convention.
  companyKey: z.string().optional(),
  roleKey: z.string().optional(),
  // Publishing gate: drafts exist locally but should not be public in production.
  publishStatus: z.enum(['draft', 'published']).optional().default('published'),
