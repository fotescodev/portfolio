...
function makeExcerpt(text: string, maxChars = 900): string {
  ...
  const cut = lastSentence > 240 ? lastSentence + 1 : slice.lastIndexOf(' ');
  return slice.slice(0, Math.max(0, cut)).trim() + '…';
}
...
function variantPathFromMeta(meta: any): string {
  const slug = String(meta.slug || '').trim();
  const companyKey = String(meta.companyKey || slug.split('-')[0] || '').trim();
  const roleKey = String(meta.roleKey || slug.split('-').slice(1).join('-') || '').trim();
...
async function main() {
  ...
  try {
    ...
    // --- Safety policy: store full JD locally (gitignored), ship only an excerpt ---
    const fullJd = String(validated.metadata.jobDescription || '').trim();
    const excerpt = makeExcerpt(fullJd, 900);
    validated.metadata.jobDescription = excerpt;

    // Default new variants to draft until the pipeline passes + you publish.
    validated.metadata.publishStatus = 'draft';

    // Ensure we have URL segments available for consistent routing.
    if (!validated.metadata.companyKey || !validated.metadata.roleKey) {
      const slug = String(validated.metadata.slug || '').trim();
      validated.metadata.companyKey = validated.metadata.companyKey || slug.split('-')[0] || slugify(validated.metadata.company);
      validated.metadata.roleKey = validated.metadata.roleKey || slug.split('-').slice(1).join('-') || slugify(validated.metadata.role);
    }
    ...
    // Write raw/full JD to source-data (gitignored)
    const jdDir = join(process.cwd(), 'source-data', 'jds');
    ensureDir(jdDir);
    const jdPath = join(jdDir, `${validated.metadata.slug}.md`);
    const jdHeader = [
      `# Job Description`,
      ``,
    ...
    writeFileSync(jdPath, jdHeader + fullJd + '\n', 'utf-8');

    // Re-stringify YAML so the saved file matches what ships (excerpt + draft status).
    yamlContent = YAML.stringify(validated, { lineWidth: 0 });
    ...
    console.log(`${colors.gray}URL:${colors.reset} ${variantPathFromMeta(validated.metadata)}`);

    console.log(`\n${colors.cyan}Next steps:${colors.reset}`);
    console.log(`  1. Review and edit: ${yamlFile}`);
    console.log(`  2. Run validation: npm run validate`);
    console.log(`  3. Test locally: npm run dev`);
    console.log(`  4. Visit: http://localhost:5173${variantPathFromMeta(validated.metadata)}`);
