...
import { HEADER_COMPACT, isInteractive, theme } from '../theme.js';
...
type BottomPanelMode = 'activity' | 'eval' | 'redteam' | 'claims';

type Overlay =
  | { kind: 'actions'; selected: number }
  | { kind: 'claims'; focus: 'claims' | 'sources'; claimIndex: number; sourceIndex: number }
  | { kind: 'help' }
  | null;
...
function fmtPath(p: string): string {
  // "clickable-ish" in many terminals + easy to copy
  return theme.underline(theme.info(p));
}
...
function renderClaimsOverlay(row: VariantRow | undefined, overlay: Extract<Overlay, { kind: 'claims' }>, width: number, height: number): string[] {
  ...
  if (ev.unverified === 0) {
    return [
      theme.bold('Verify claims'),
      '',
      theme.success('All claims are verified.'),
      theme.muted('You can proceed to Redteam → Publish.')
    ];
  }
  ...
  lines.push(theme.muted('Tab switches focus. Enter verifies using selected source.'));
  lines.push('');
...
function renderHelpPanel(width: number): string[] {
  ...
  lines.push(`${theme.icons.bullet} p  Cycle bottom panel (activity/eval/redteam/claims)`);
  lines.push(`${theme.icons.bullet} ${accel}1..4  Jump panels (1=activity, 2=eval, 3=redteam, 4=claims)`);
  lines.push('');
...
async function extractJobFromUrl(url: string): Promise<JobExtraction> {
  try {
    ...
    // Fallback: headless render (many job boards are JS-heavy)
...
async function fetchHtml(url: string): Promise<string> {
  const res = await fetch(url, {
    headers: {
      'User-Agent': 'Mozilla/5.0 (ucv-cli)'
    }
  });
...
async function main() {
  ...
  async function verifyClaimFromOverlay() {
    ...
    try {
      ...
      // Keep the overlay open for additional verification.
      overlay = { kind: 'claims', focus: 'claims', claimIndex: 0, sourceIndex: 0 };
      bottomMode = 'claims';
      render();
    } finally {
  ...
  process.stdin.on('keypress', async (str, key: any) => {
    ...
    // intercept keys (except Ctrl+C and q handled above).
    if (busy) {
      return;
    }
    ...
    if (accel && ['1', '2', '3', '4'].includes(key.name)) {
