...
## Phase 4: Publish gate (draft → public)

Variants are created as **drafts**. They only become visible in production after
they pass the pipeline and you explicitly publish them.

### Metadata fields

- `metadata.publishStatus`: `draft` | `published`
- `metadata.publishedAt`: ISO timestamp (set when publishing)

### Safety: full JD local, excerpt shipped

- Full JD: `source-data/jds/<slug>.md` *(gitignored)*
- Excerpt: `metadata.jobDescription` *(kept short; shipped to the site)*

### Publish with ucv-cli

```bash
npm run ucv-cli
```

In the action menu: **Publish** flips the status + syncs JSON + (optionally)
commits/pushes.
